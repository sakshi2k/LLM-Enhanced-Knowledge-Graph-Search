...
# Load CSV
print('Loading CSV')
df = pd.read_csv('./all_ECB_speeches.csv', delimiter='|', dtype=str)
df.fillna('', inplace=True)

# Build speaker canonical map
speakers = sorted(set([s.strip() for s in df['speakers'].unique() if s.strip()!='']))
def norm_name(n):
...
def detect_intent(query_lower):
    intents = [
        ...
        ("timeline", ["trend", "evolution", "how did", "how has", "evolve", "after", "before", "since"]),
    ...
    return {"label":"fact_lookup","confidence":0.5}
