...
class KnowledgeGraphBuilder:
    ...
    def _get_or_create_node(
        ...
        **attrs,
        ...
        if key in self.node_index:
            ...
            # Preserve existing source_row if present, otherwise use new one
            if "source_row" in attrs and "source_row" not in self.graph.nodes[node_id]:
                self.graph.nodes[node_id]["source_row"] = attrs["source_row"]
            # Update other attributes
            for k, v in attrs.items():
                if k != "source_row" or "source_row" not in self.graph.nodes[node_id]:
    ...
    def upsert_topic(self, topic: TopicCandidate, source_row: Optional[int] = None) -> str:
        vector = topic.embedding
        # Clamp confidence to [0, 1] range to avoid floating point precision issues
        confidence = min(1.0, max(0.0, topic.score))
        ...
        if best_score >= self.topic_threshold:
            ...
            # Average confidence and clamp to [0, 1]
            old_conf = self.graph.nodes[topic_id].get("confidence", confidence)
            new_conf = min(1.0, max(0.0, (old_conf + confidence) / 2))
            self.graph.nodes[topic_id]["confidence"] = new_conf
            # Track source_row if not already set
            if source_row is not None and "source_row" not in self.graph.nodes[topic_id]:
                self.graph.nodes[topic_id]["source_row"] = source_row
    ...
    def add_speech(
        ...
        topic_ids: List[str] = []
        for topic in topics:
            topic_id = self.upsert_topic(topic, source_row=row_id)
            topic_ids.append(topic_id)
            # Clamp edge confidence to [0, 1]
            edge_confidence = min(1.0, max(0.0, topic.score))
            self.graph.add_edge(
                speaker_id,
                topic_id,
    ...
    def export(self, output_dir: str, neo4j_uri: Optional[str] = None, neo4j_user: Optional[str] = None, neo4j_password: Optional[str] = None):
        """
        Export graph to CSV files (and optionally directly to Neo4j).
        
        CSV export is recommended for large datasets as Neo4j's bulk CSV import
        is much faster than individual transactions.
        """
        os.makedirs(output_dir, exist_ok=True)
        nodes_path = os.path.join(output_dir, "nodes.csv")
        edges_path = os.path.join(output_dir, "edges.csv")
        
        # Export nodes, removing 'source' column if present
        nodes_data = []
        for node_id, data in self.graph.nodes(data=True):
            node_dict = {"id": node_id, **data}
            # Remove 'source' column if it exists (legacy from old code)
            node_dict.pop("source", None)
            nodes_data.append(node_dict)
        nodes_df = pd.DataFrame(nodes_data)
        nodes_df.to_csv(nodes_path, index=False, quoting=csv.QUOTE_NONNUMERIC)
        ...
        edges_df.to_csv(edges_path, index=False, quoting=csv.QUOTE_NONNUMERIC)
        
        # Optional: Direct Neo4j export (slower but convenient for small datasets)
        if neo4j_uri:
            try:
                ...
                with driver.session() as session:
                    ...
                    for _, row in nodes_df.iterrows():
                        ...
                        props = {k: v for k, v in row.items() if k != "type" and pd.notna(v)}
                    ...
                    for _, row in edges_df.iterrows():
                        ...
                        props = {k: v for k, v in row.items() if k not in ["source", "target", "type", "key"] and pd.notna(v)}
                        query = f"""
                        MATCH (a {{id: $source}}), (b {{id: $target}})
                        CREATE (a)-[r:{rel_type} $props]->(b)
                        """
                        session.run(query, source=row["source"], target=row["target"], props=props)
                
                driver.close()
                print(f"\n✓ Graph also exported directly to Neo4j at {neo4j_uri}")
            except ImportError:
                print("\n⚠ Neo4j driver not installed. Install with: pip install neo4j")
            except Exception as e:
                print(f"\n⚠ Neo4j export failed: {e}")
                print("   CSV files are still available for manual Neo4j import")
